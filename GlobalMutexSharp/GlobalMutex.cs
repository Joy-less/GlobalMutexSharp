...
public sealed class GlobalMutex : IDisposable {
    ...
    private bool TryEnter(TimeSpan Timeout) {
        lock (Lock) {
            ...
            try {
                ...
                // The mutex was abandoned in another process, but it will still get acquired
                return true;
            }
        }
    }
    ...
    public readonly struct DisposeExiter(GlobalMutex Mutex) : IDisposable {
        public GlobalMutex Mutex { get; } = Mutex;
